...
<body class="bg-slate-50 text-slate-800 font-sans antialiased">
    ...
    <!-- SIDEBAR -->
    <aside id="sidebar" class="fixed top-0 left-0 h-screen w-60 bg-slate-900 flex flex-col z-30
           transition-transform duration-300 lg:translate-x-0 -translate-x-full" aria-label="Navigation principale">

        <!-- Logo -->
        <div class="flex items-center gap-3 px-5 py-5 border-b border-slate-800">
            <div
                class="flex-shrink-0 w-9 h-9 rounded-xl bg-violet-600 flex items-center justify-center shadow-lg shadow-violet-900/50">
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-2 space-y-0.5" aria-label="Menu">
            ...
            <?php if (!empty($droits['approvisionnement.consulter'])): ?>
                <div class="pt-3 pb-1 px-3">
                    <span
                        class="text-[10px] uppercase tracking-widest text-slate-600 font-semibold">Approvisionnement</span>
                </div>
                ...
                <?= navItem('/approvisionnement/rapports', 'fa-solid fa-chart-bar', 'Rapports', $currentUri) ?>
            <?php endif; ?>
            ...
            <div class="flex items-center gap-2">
                ...
                <?php if ($nbAlertes > 0): ?>
                    <a href="<?= url('structure/produits?alerte=1') ?>"
                        ...
                        <span
                            class="absolute -top-0.5 -right-0.5 w-4 h-4 bg-rose-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                ...
                <?= $type === 'error' ? 'bg-rose-50 border border-rose-200 text-rose-700'
                    : ($type === 'success' ? 'bg-emerald-50 border border-emerald-200 text-emerald-700'
                        : 'bg-blue-50 border border-blue-200 text-blue-700') ?>">
                            <i
                                class="fa-solid <?= $type === 'error' ? 'fa-circle-exclamation' : ($type === 'success' ? 'fa-circle-check' : 'fa-circle-info') ?> mt-0.5 flex-shrink-0"></i>
