<?php
$lignesExistantes = $cmd['lignes'] ?? [];
?>
<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        <a href="<?= url('approvisionnement/commandes') ?>" class="btn-secondary btn-sm"><i class="fa-solid fa-arrow-left"></i></a>
        <h1 class="text-xl font-bold text-slate-800"><?= $cmd ? 'Modifier la commande' : 'Nouvelle commande fournisseur' ?></h1>
    </div>
    ...
    <form method="POST" action="<?= $cmd ? url('approvisionnement/commandes/'.$cmd['oid_doc'].'/edit') : url('approvisionnement/commandes/creer') ?>">
        <?= csrfField() ?>
        <div class="card card-body mb-4">
            <?php if(!$cmd): ?>
            <div class="mb-4">
                <label class="form-label">Fournisseur <span class="text-rose-500">*</span></label>
                <select name="id_fournisseur" class="form-select <?= !empty($errors['id_fournisseur'])?'border-rose-400':'' ?>" required>
                    <option value="">— Choisir —</option>
                    <?php foreach($fournisseurs as $f): ?>
                    <option value="<?= $f['id_fournisseur'] ?>"><?= e($f['nom']) ?></option>
                    <?php endforeach; ?>
                </select>
                <?php if(!empty($errors['id_fournisseur'])): ?><p class="form-error"><?= e($errors['id_fournisseur']) ?></p><?php endif; ?>
            </div>
            <?php else: ?>
            <div class="mb-4 text-sm text-slate-500">Fournisseur : <span class="font-semibold text-slate-700"><?= e($cmd['fournisseur']) ?></span> (non modifiable)</div>
            <?php endif; ?>
            <div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm" id="tableLignes">
                    <thead><tr class="bg-slate-50 text-xs text-slate-500 uppercase tracking-wide">
                        ...
                        <th class="px-4 py-2.5 text-right w-36">Prix unit.</th>
        ...
        <div class="flex justify-end gap-3">
            <a href="<?= url('approvisionnement/commandes') ?>" class="btn-secondary">Annuler</a>
...
const LIGNES_EXISTANTES = <?= json_encode(array_map(fn($l)=>['id_produit'=>$l['id_produit'],'quantite'=>$l['quantite'],'prix_unitaire'=>$l['prix_unitaire']], $lignesExistantes)) ?>;
let ligneIndex = 0;

function optionsProduits(selectedId = '') {
    return '<option value="">— Produit —</option>' + PRODUITS.map(p =>
        `<option value="${p.id}" data-prix="${p.prix_achat}" data-unite="${p.unite}" ${String(p.id)===String(selectedId)?'selected':''}>${p.code} — ${p.designation}</option>`
    ).join('');
}

function ajouterLigne(idProduit = '', quantite = '', prixUnitaire = '') {
    const idx = ligneIndex++;
    const tr = document.createElement('tr');
    tr.className = 'border-t border-slate-100';
    tr.innerHTML = `
        <td class="px-4 py-2">
            <select name="produit_id[]" class="form-select py-1.5 text-sm" onchange="majPrix(this, ${idx})">
                ${optionsProduits(idProduit)}
            </select>
        </td>
...
function majPrix(select, idx) {
    ...
    if (opt && opt.dataset.prix) {
        document.getElementById('prix_' + idx).value = opt.dataset.prix;
        calculerLigne(idx);
    }
}
...
function calculerTotal() {
    let total = 0;
    document.querySelectorAll('[id^="montant_"]').forEach(el => {
...
if (LIGNES_EXISTANTES.length > 0) {
    LIGNES_EXISTANTES.forEach(l => ajouterLigne(l.id_produit, l.quantite, l.prix_unitaire));
} else {
    ajouterLigne();
}
</script>
