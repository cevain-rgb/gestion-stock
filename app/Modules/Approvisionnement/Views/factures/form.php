<div class="max-w-xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        <a href="<?= url('approvisionnement/factures') ?>" class="btn-secondary btn-sm"><i class="fa-solid fa-arrow-left"></i></a>
        <h1 class="text-xl font-bold text-slate-800">Nouvelle facture fournisseur</h1>
    </div>
    ...
    <div class="card card-body text-center py-8 text-slate-400">
        ...
        Aucune commande entièrement reçue n'est en attente de facturation.
    </div>
    <?php else: ?>
    <div class="card card-body">
        <form method="POST" action="<?= url('approvisionnement/factures/creer') ?>">
            <?= csrfField() ?>
            <div class="mb-4">
                <label class="form-label">Commande <span class="text-rose-500">*</span></label>
                <select name="id_commande_f" id="selectCmd" class="form-select <?= !empty($errors['id_commande_f'])?'border-rose-400':'' ?>" required onchange="majMontant()">
                    <option value="">— Choisir une commande reçue —</option>
                    <?php foreach($commandes as $c): ?>
                    <option value="<?= $c['oid_doc'] ?>" data-montant="<?= $c['montant_total'] ?>"><?= e($c['numero'].' — '.$c['fournisseur'].' ('.number_format((float)$c['montant_total'],0,',',' ').' FCFA)') ?></option>
                    <?php endforeach; ?>
                </select>
                <?php if(!empty($errors['id_commande_f'])): ?><p class="form-error"><?= e($errors['id_commande_f']) ?></p><?php endif; ?>
            </div>
            ...
            <div class="flex justify-end gap-3">
                <a href="<?= url('approvisionnement/factures') ?>" class="btn-secondary">Annuler</a>
                <button type="submit" class="btn-primary"><i class="fa-solid fa-file-invoice"></i> Créer la facture</button>
            </div>
        </form>
    </div>
    <?php endif; ?>
...
function majMontant() {
    ...
    if (opt && opt.dataset.montant) {
        document.getElementById('montantHt').value = opt.dataset.montant;
        majTtc();
    }
}
function majTtc() {
    ...
    document.getElementById('montantTtc').textContent = ttc.toLocaleString('fr-FR') + ' FCFA';
}
</script>
