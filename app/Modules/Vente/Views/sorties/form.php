<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <h1 class="text-xl font-bold text-slate-800">Nouveau bon de sortie</h1>
    </div>
    ...
    <form method="POST" action="<?= url('vente/sorties/creer') ?>">
        <?= csrfField() ?>
        <div class="card card-body mb-4">
            <div class="mb-4">
                <label class="form-label">Motif <span class="text-rose-500">*</span></label>
                <select name="motif" class="form-select <?= !empty($errors['motif'])?'border-rose-400':'' ?>" required>
                ...
                <?php if(!empty($errors['motif'])): ?><p class="form-error"><?= e($errors['motif']) ?></p><?php endif; ?>
            </div>
            <div>
        ...
        <div class="card overflow-hidden mb-4">
            <div class="card-header">
                <h2 class="font-semibold text-slate-700">Articles concernés</h2>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm" id="tableLignes">
                    <thead><tr class="bg-slate-50 text-xs text-slate-500 uppercase tracking-wide">
                        ...
                        <th class="px-4 py-2.5 text-right w-32">Valeur unit.</th>
                        <th class="px-4 py-2.5 text-left">Détail</th>
            ...
            <div class="px-4 py-3 bg-slate-50 border-t border-slate-100 flex justify-end">
                ...
                <span class="text-lg font-bold text-rose-700" id="totalGeneral">0 FCFA</span>
            </div>
        </div>

        <div class="flex justify-end gap-3">
            <a href="<?= url('vente/sorties') ?>" class="btn-secondary">Annuler</a>
            <button type="submit" class="btn-primary"><i class="fa-solid fa-box-open"></i> Enregistrer le bon de sortie</button>
        </div>
    </form>
</div>

<script>
const PRODUITS = <?= json_encode(array_map(fn($p)=>['id'=>$p['id_produit'],'code'=>$p['code'],'designation'=>$p['designation'],'unite'=>$p['unite'],'prix_achat'=>$p['prix_achat'],'stock'=>$p['stock_actuel']], $produits)) ?>;
let ligneIndex = 0;

function optionsProduits() {
    return '<option value="">— Produit —</option>' + PRODUITS.map(p =>
        `<option value="${p.id}" data-prix="${p.prix_achat}">${p.code} — ${p.designation} (stock: ${parseFloat(p.stock).toLocaleString('fr-FR')})</option>`
    ).join('');
}
...
function ajouterLigne() {
    ...
    tr.innerHTML = `
        ...
        <td class="px-4 py-2"><input type="text" name="motif_detail[]" class="form-input py-1.5 text-sm" placeholder="Précision..."></td>
...
function majPrix(select, idx) {
    ...
    if (opt && opt.dataset.prix) { document.getElementById('val_' + idx).value = opt.dataset.prix; calculerLigne(idx); }
}
