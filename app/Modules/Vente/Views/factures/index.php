...
<div class="flex items-center justify-between mb-6">
    <div><h1 class="text-xl font-bold text-slate-800">Factures clients</h1><p class="text-sm text-slate-500"><?= $total ?> facture(s)</p></div>
    <?php if(!empty($_SESSION['droits']['vente.creer'])): ?>
    <a href="<?= url('vente/factures/creer') ?>" class="btn-primary"><i class="fa-solid fa-plus"></i> Nouvelle facture</a>
    <?php endif; ?>
</div>
<div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
<?php
$kpis=[['Total',$stats['total']??0,'fa-file-invoice-dollar','bg-slate-50 text-slate-600'],
       ...
       ['Créances',money($stats['total_du']??0),'fa-money-bill-wave','bg-violet-50 text-violet-600']];
...
<div class="card card-body mb-4">
    <form method="GET" action="<?= url('vente/factures') ?>" class="flex flex-wrap items-end gap-3">
        <div class="flex-1 min-w-[160px]"><label class="form-label text-xs">Recherche</label>
        <input type="text" name="q" value="<?= e($filtres['recherche']??'') ?>" placeholder="N° facture, client..." class="form-input py-2 text-sm"></div>
        ...
        <a href="<?= url('vente/factures') ?>" class="btn-secondary py-2"><i class="fa-solid fa-xmark"></i></a>
    </form>
</div>
<div class="card overflow-hidden">
    <table class="data-table"><thead><tr><th>N° Facture</th><th>Client</th><th>Date</th><th class="text-right">Montant TTC</th><th class="text-right">Reste</th><th class="text-center">Statut</th><th class="text-center">Actions</th></tr></thead>
    <tbody>
    ...
    <tr>
        ...
        <td class="font-medium text-slate-700"><?= e($f['client']) ?></td>
        ...
        <td><div class="flex items-center justify-center gap-1">
            <a href="<?= url('vente/factures/'.$f['oid_doc']) ?>" class="btn-secondary btn-sm"><i class="fa-solid fa-eye"></i></a>
        </div></td>
    </tr>
    <?php endforeach; ?>
    </tbody></table>
</div>
<?= paginationLinks($page??1, $totalPages, url('vente/factures').'?'.http_build_query(array_filter($filtres??[]))) ?>
